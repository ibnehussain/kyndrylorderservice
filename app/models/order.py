...
class OrderItem(BaseModel):
    ...
    def validate_total_price(cls, v, values):
        ...
        if "quantity" in values and "unit_price" in values:
            ...
            if v != expected_total:
                raise ValueError(
                    f"Total price {v} must equal quantity {values['quantity']} * unit_price {values['unit_price']}"
                )
        return v
...
class Order(TimestampMixin):
    ...
    def validate_total_amount(cls, v, values):
        ...
        if all(
            ...
            if abs(v - expected_total) > Decimal(
                "0.01"
    ...
    def validate_subtotal(cls, v, values):
        ...
        if "items" in values:
            ...
            if abs(v - calculated_subtotal) > Decimal("0.01"):
                raise ValueError(
                    f"Subtotal {v} does not match sum of items {calculated_subtotal}"
                )
        return v
