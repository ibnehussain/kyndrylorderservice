...
from app.models.analytics import DailyOrderMetrics, OrderStatusMetrics, CustomerMetrics
...
class OrderRepository(BaseRepository[Order]):
    ...
    def _order_from_dict(self, data: Dict[str, Any]) -> Order:
        ...
        for field in ['subtotal', 'tax_amount', 'shipping_amount', 'discount_amount', 'total_amount']:
            if field in data:
    ...
    async def list_items(self, customer_id: Optional[str] = None, 
                        max_items: int = 100, 
    ...
    async def get_order_by_number(self, order_number: str) -> Optional[Order]:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to get order by number {order_number}: {str(e)}")
    
    # Analytics Methods
    
    async def get_daily_metrics(self, start_date: date, end_date: date) -> List[DailyOrderMetrics]:
        ...
        try:
            ...
            daily_metrics = []
            for item in items:
                daily_metrics.append(DailyOrderMetrics(
    ...
    async def get_order_status_metrics(self, start_date: date, end_date: date) -> List[OrderStatusMetrics]:
        ...
        try:
            ...
            for item in items:
                ...
                status_metrics.append(OrderStatusMetrics(
                    status=item['status'],
                    count=item['count'],
    ...
    async def get_customer_metrics(self, start_date: date, end_date: date, 
                                 limit: int = 10) -> List[CustomerMetrics]:
        ...
        try:
            ...
            for item in items:
                ...
                if item.get('first_order_date'):
                    first_order_date = datetime.fromisoformat(item['first_order_date'].replace('Z', '+00:00'))
                if item.get('last_order_date'):
                    last_order_date = datetime.fromisoformat(item['last_order_date'].replace('Z', '+00:00'))
                
                customer_metrics.append(CustomerMetrics(
    ...
    async def get_revenue_summary(self, start_date: date, end_date: date) -> Tuple[Decimal, int, Decimal]:
    ...
    async def get_busiest_day(self, start_date: date, end_date: date) -> Optional[Tuple[date, int]]:
    ...
    async def get_highest_revenue_day(self, start_date: date, end_date: date) -> Optional[Tuple[date, Decimal]]:
