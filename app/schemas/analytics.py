...
class DailyAnalyticsResponse(BaseModel):
    ...
    metrics: List[DailyOrderMetrics] = Field(..., description="Daily metrics for the requested period")
    period_summary: RevenueMetrics = Field(..., description="Summary metrics for the entire period")
...
class OrderStatusAnalyticsResponse(BaseModel):
    ...
    status_metrics: List[OrderStatusMetrics] = Field(..., description="Metrics grouped by order status")
    period: AnalyticsDateRange = Field(..., description="Date range for the analytics")
    total_orders: int = Field(..., ge=0, description="Total number of orders in the period")
    total_revenue: Decimal = Field(..., ge=0, decimal_places=2, description="Total revenue in the period")
...
class TopCustomersResponse(BaseModel):
    ...
    customers: List[CustomerMetrics] = Field(..., description="Top customers by total spent")
    period: AnalyticsDateRange = Field(..., description="Date range for the analytics")
...
class AnalyticsSummaryResponse(BaseModel):
    ...
    status_breakdown: List[OrderStatusMetrics] = Field(..., description="Orders by status")
    daily_trend: List[DailyOrderMetrics] = Field(..., description="Daily metrics trend")
    top_customers: List[CustomerMetrics] = Field(..., description="Top 5 customers by spending")
    
    # Additional summary fields
    growth_rate: Optional[float] = Field(None, description="Revenue growth rate compared to previous period")
    busiest_day: Optional[date] = Field(None, description="Day with highest order count")
    highest_revenue_day: Optional[date] = Field(None, description="Day with highest revenue")
...
class AnalyticsQueryParams(BaseModel):
    ...
    start_date: Optional[date] = Field(None, description="Start date (defaults to 30 days ago)")
    ...
    def get_date_range(self) -> AnalyticsDateRange:
        ...
        start_date = self.start_date or date.today().replace(day=1)  # First day of current month
